...
static int htc_char_width;
static int htc_char_height;
bool showWVZFRmessage = false;
...
#ifdef VENDOR_VERIZON
        if (currentIcon == INSTALLING_UPDATE) {
            int iconX = (gr_fb_width() - gr_get_width(backgroundIcon[VERIZON_STRING])) / 2;
            int iconY = (gr_fb_height() + gr_get_height(loopFrames[0])) / 2;
            gr_blit(backgroundIcon[VERIZON_STRING], 0, 0, gr_get_width(backgroundIcon[VERIZON_STRING]), gr_get_height(backgroundIcon[VERIZON_STRING]), iconX, iconY);
        }
#endif
...
void ScreenRecoveryUI::draw_foreground_locked() {
  if (currentIcon != NONE) {
#if HTC_HEP_PROJECT_FLAG
    static double start = 0;
    static double interval = 1.2;
    static double end = 0;
    if (currentIcon == INSTALLING_UPDATE || currentIcon == ERASING) {
      GRSurface* icon = NULL;
      if ( IsKeyPressed(KEY_POWER) ) {
        icon = backgroundIcon[INSTALLING_WARNING];
        start = now();
      } else {
        end = now();
        if ( (end - start) >= interval ) {
          icon = GetCurrentFrame();
        } else {
          icon = backgroundIcon[INSTALLING_WARNING];
        }
      }
      gr_blit(icon, 0, 0, gr_get_width(icon), gr_get_height(icon), (gr_fb_width() - gr_get_width(icon)) / 2, GetAnimationBaseline());
    } else {
...
void ScreenRecoveryUI::draw_screen_locked() {
    if (!show_text) {
        ...
        gr_color(0, 0, 0, 255);
        gr_clear();
        int y = 0 + aod_height;//Support Always-On Display (sub-screen) feature
        if (show_menu) {
        ...
        if (showWVZFRmessage) {
            SetColor(HEADER);
            DrawTextLine(TEXT_INDENT, &y, "If your phone is reset, you'll", true);
            DrawTextLine(TEXT_INDENT, &y, "need to enter the Google account", true);
            DrawTextLine(TEXT_INDENT, &y, "information (email address and", true);
            DrawTextLine(TEXT_INDENT, &y, "password) associated with this", true);
            DrawTextLine(TEXT_INDENT, &y, "phone to be able to use it", true);
            DrawTextLine(TEXT_INDENT, &y, "again.", true);
        }
...
void ScreenRecoveryUI::show_percentage()
{
    if (progressBarType == DETERMINATE) {
#ifdef HTC_HEP_PROJECT_FLAG
        // align with L SHEP
        int iconHeight = 0;
#else
        // Original AOSP design
        ...
        char percentage[5] = {0};
        snprintf(percentage, sizeof(percentage), "%d%%", (int)(p*100));
        int dx = (gr_fb_width() - (strlen(percentage)*htc_char_width))/2;
        int dy = (3*gr_fb_height() + iconHeight - 2*height)/4;
        gr_color(0, 0, 0, 255);
        gr_fill(dx, dy+(htc_char_height*3/2), dx+(htc_char_width*4), (dy+(htc_char_height*3/2))+htc_char_height);
        SetColor(MENU_SEL_FG);
        htc_gr_text(dx, dy+(htc_char_height*3/2), percentage, 1);
    }
}
...
bool ScreenRecoveryUI::InitTextParams() {
    ...
    htc_gr_font_size(&htc_char_width, &htc_char_height);
...
bool ScreenRecoveryUI::Init(const std::string& locale) {
...
#if HTC_HEP_PROJECT_FLAG
  LoadBitmap("icon_install_corrupt",&backgroundIcon[NO_COMMAND]);
  LoadBitmap("fota_icon_installing_warning",&backgroundIcon[INSTALLING_WARNING]);
  //indeterminate progress bar (used in wipe_cache flow)
  LoadBitmap("indeterminate1", &progressBarIndeterminate[0]);
  LoadBitmap("indeterminate2", &progressBarIndeterminate[1]);
  LoadBitmap("indeterminate3", &progressBarIndeterminate[2]);
  LoadBitmap("indeterminate4", &progressBarIndeterminate[3]);
  LoadBitmap("indeterminate5", &progressBarIndeterminate[4]);
  LoadBitmap("indeterminate6", &progressBarIndeterminate[5]);
  //determinate progress bar (used in update flow)
  backgroundIcon[NONE] = nullptr;
  LoadBitmap("icon_error", &backgroundIcon[ERROR]);
#ifdef VENDOR_VERIZON
  LoadBitmap("verizon_string", &backgroundIcon[VERIZON_STRING]);
#endif
  LoadBitmap("progress_bar_empty", &progressBarEmpty);
  LoadBitmap("progress_bar_fill", &progressBarFill);
#else
  ...
  // suport sub_screen feature for OCE_NOTE
  FILE *pFile;
  char buffer[1024];
  memset(buffer,0,1024);
  aod_height = 0;
  pFile = fopen( "/sys/class/graphics/fb0/msm_fb_panel_info", "r" );
  if ( NULL != pFile ){
      while (fscanf(pFile, "%1023s", buffer) != EOF){
          if(strncmp(buffer, "aod_height", 10) == 0){
              aod_height = atoi(buffer + 11);
              aod_height = (aod_height  > 0 && aod_height <= gr_fb_height()) ? aod_height : 0;
          }
      }
      fclose(pFile);
  }
...
int ScreenRecoveryUI::SelectMenu(int sel) {
    ...
    if (show_menu) {
        ...
        sel = menu_sel;
        char frp[PROPERTY_VALUE_MAX] = {0};
        property_get("ro.frp.pst", frp, "");
        // sel start from 0 and will be increased to 4 when highlight the item
        // to "Wipe data/factory reset"
        // but Device::WIPE_DATA is 5, so sel+1
        if (sel+1 == Device::WIPE_DATA && strlen(frp) != 0) {
            showWVZFRmessage = false;
        } else {
            showWVZFRmessage = false;
        }
...
// ==================================
// Help functions for display
// ==================================
static void convert_ARGB8888_to_RGB565(unsigned int *src, unsigned short *dst, int LCD_XRES, int LCD_YRES, int Fixed_LCD_XRES) {
    int x,y;
    unsigned int tempvalue;
    unsigned int a,r,g,b;
    int fix_size = Fixed_LCD_XRES - LCD_XRES;

    for (y = 0 ; y < LCD_YRES ; y++) {
        for (x = 0 ; x < LCD_XRES ; x++) {
            tempvalue = *src;
            a = (tempvalue & 0xff000000)>>24;
            r = (tempvalue & 0x00ff0000)>>16;
            g = (tempvalue & 0x0000ff00)>>8;
            b = (tempvalue & 0x000000ff)>>0;
            *dst = ((b>>3)<<11) | (((g>>3)<<1)<<5) | (r>>3);
...
static void copy_and_adjust_image (unsigned short *src, unsigned short *dst, int LCD_XRES, int LCD_YRES, int Fixed_LCD_XRES) {
    int x, y;
    int fix_size = Fixed_LCD_XRES - LCD_XRES;
    for (y = 0 ; y < LCD_YRES ; y++) {
        for (x = 0 ; x < LCD_XRES ; x++) {
...
char* ScreenRecoveryUI::DumpFramebuffer (Icon icon, int *width, int *height, int *bpp) {
    int size, rsize;
    int rwidth, rheight, rbytepp;
    char *buffer, *rbuffer;
    ...
    // For recovery image
    rwidth = gr_fb_width_modified();
    rheight = gr_fb_height();
    rbytepp = gr_fb_pixel();

    // For hboot image, 16bpp
    *width = gr_fb_width_original();
    *height = gr_fb_height();
    *bpp = 16; // 16 bits

    if (gr_fb_pixel() == 4) {
        // Recovery image, 32bpp
        rsize = rwidth * rheight * rbytepp;
        size = rsize / 2;
        rbuffer = (char*)malloc(rsize);
        buffer = (char*)malloc(size);
        if (rbuffer == NULL || buffer == NULL) {
            LOG(ERROR) << "Can't allocate " << size << " bytes for image";
            if (rbuffer != NULL) free(rbuffer);
            if (buffer != NULL) free(buffer);
        } else {
            memcpy(rbuffer, gr_fb_data(), rsize);
            convert_ARGB8888_to_RGB565((unsigned int*)rbuffer, (unsigned short*)buffer, *width, *height, rwidth);
            free(rbuffer);
        }
    } else {
        // Recovery image, 16bpp
        rsize = rwidth * rheight * rbytepp;
        ...
        if (rbuffer == NULL || buffer == NULL) {
            ...
            if (rsize == size) {
                memcpy(buffer, gr_fb_data(), size);
            } else {
                memcpy(rbuffer, gr_fb_data(), rsize);
                copy_and_adjust_image((unsigned short*)rbuffer, (unsigned short*)buffer, *width, *height, rwidth);
            }
            free(rbuffer);
        }
    }
