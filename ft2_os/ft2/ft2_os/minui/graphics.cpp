...
static GRFont* htc_gr_font = NULL;
...
void htc_gr_font_size(int *x, int *y)
{
    *x = htc_gr_font->char_width;
    *y = htc_gr_font->char_height;
}
...
void htc_gr_text(int x, int y, const char *s, bool bold)
{
    GRFont* font = htc_gr_font;
...
static void htc_gr_init_font(void) {
    htc_gr_font = reinterpret_cast<GRFont*>(calloc(sizeof(*htc_gr_font), 1));

    int res = res_create_alpha_surface("htc_font", &(htc_gr_font->texture));
    if (res == 0) {
        // The font image should be a 96x2 array of character images.  The
        // columns are the printable ASCII characters 0x20 - 0x7f.  The
        // top row is regular text; the bottom row is bold.
        htc_gr_font->char_width = htc_gr_font->texture->width / 96;
        htc_gr_font->char_height = htc_gr_font->texture->height / 2;
    } else {
        // Load htc font fail
        printf("failed to read htc font: res=%d\n", res);

        // fall back to the compiled-in font.
        htc_gr_font->texture = reinterpret_cast<GRSurface*>(malloc(sizeof(*gr_font->texture)));
        htc_gr_font->texture->width = font.width;
        htc_gr_font->texture->height = font.height;
        htc_gr_font->texture->row_bytes = font.width;
        htc_gr_font->texture->pixel_bytes = 1;

        unsigned char* bits = reinterpret_cast<unsigned char*>(malloc(font.width * font.height));
        htc_gr_font->texture->data = reinterpret_cast<unsigned char*>(bits);
        ...
        htc_gr_font->char_width = font.char_width;
        htc_gr_font->char_height = font.char_height;
    }
}
...
int gr_fb_width_modified()
{
    return gr_draw->row_bytes * 8 / (gr_draw->pixel_bytes * 8);
}

int gr_fb_width_original()
{
    return gr_draw->width;
}

int gr_fb_pixel(void)
{
...
{
    return (unsigned short *) gr_draw->data;
}
