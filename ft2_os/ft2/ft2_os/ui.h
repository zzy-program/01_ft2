...
class RecoveryUI {
  public:
    ...
    enum Icon { NONE, INSTALLING_UPDATE, ERASING, NO_COMMAND, ERROR, FACTORYRESET, FIRMWARE_INSTALLING, INSTALLING_WARNING, VERIZON_STRING, ICON_NUM };
#else
    enum Icon { NONE, INSTALLING_UPDATE, ERASING, NO_COMMAND, ERROR, ICON_NUM };
#endif
...
#ifdef HTC_HEP_PROJECT_FLAG
    // --- progress bar ---
    static const int PROGRESSBAR_STATES = 6;
#endif
    ...
    // DumpFramebuffer
    virtual char* DumpFramebuffer(Icon icon, int *width, int *height, int *bpp) { return NULL; };

    virtual void SetEnableToggle(bool enabled);
