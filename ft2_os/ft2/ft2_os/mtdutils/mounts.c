...
unmount_mounted_volume(const MountedVolume *volume)
{
     ...
     */
    int ret;
    if(strncmp(volume->mount_point,"/sdcard",strlen("/sdcard"))==0){
        //SD Card or USB Dongle
        ret=sd_umount_partition(volume->device,volume->mount_point);
    }else{
        //EMMC
        ret=emmc_umount_partition(volume->device,volume->mount_point);
    }
    if (ret == 0) {
